...
| --- | --- | --- | --- | --- | --- |
| parameter | Name of one model control or scenario parameter family. | `sector_activity_10kt`, `conversion_factor`, `adoption_share`, `route_share`, `structural_share`, `supply_scenario`, `capacity_basis`, `flow_scale`, `capacity_relaxation_factor`, `transport_cost_per_km` | Blank forbidden. | Author-defined public model configuration. | Figures 4-5 and model audits |
...
| value | Numeric value or text mapping for the parameter. | Fraction, factor, 10kt/y, integer, relative cost, or `same_pipeline_task_10kt` capacity-basis code. | Blank forbidden. | Direct configuration value; no private source dependency. | Figures 4-5 |
| unit | Unit or code interpretation for `value`. | `fraction`, `factor`, `10kt/y`, `integer`, `field`, `text`, or `relative_cost`. | Blank forbidden. | Declared alongside every value to prevent silent unit conversion. | Figures 4-5 |
| notes | Short semantic note and boundary. | Free text. | Blank forbidden. | Author documentation. | Figures 4-5 and audit JSON |

The public configuration intentionally has no active `transport_emission_per_km`
